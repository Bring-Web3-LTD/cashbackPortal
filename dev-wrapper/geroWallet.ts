/**
 * GeroWallet (Cardano, CIP-30) adapter that satisfies the same shape as
 * `MockWallet`, so the dev wrapper's `portalBridge` can drive the real
 * GeroWallet extension instead of a mock when the partner platform is GERO.
 *
 * Provider surface used (CIP-30, injected by GeroWallet):
 *   await window.cardano.gerowallet.enable()  -> CIP-30 `api`
...
interface GeroInitial {
    name?: string
    apiVersion?: string
    isEnabled(): Promise<boolean>
    enable(): Promise<GeroCip30Api>
}

// `window.cardano` is already declared globally by the Yoroi adapter; both
// wallets share the same CIP-30 injection point (`window.cardano.<id>`). We
// read GeroWallet's namespace off it via a local cast to avoid a conflicting
...
export interface GeroWalletOptions {
    onSign?: (info: { message: string; signature: string; key: string }) => void
}

export function createGeroWallet(opts: GeroWalletOptions = {}): MockWallet {
    const getInitial = (): GeroInitial => {
        const g = (window.cardano as Record<string, GeroInitial | undefined> | undefined)?.gerowallet
        if (!g) {
            throw new Error('GeroWallet not detected. Install the Gero extension and reload.')
        }
        return g
    }

    let api: GeroCip30Api | null = null
    let address: string | null = null // bech32
    let addressHex: string | null = null // raw hex needed for signData

    const ensureApi = async (): Promise<GeroCip30Api> => {
        if (api) return api
        api = await getInitial().enable()
        return api
    }

    const refreshAddress = async (a: GeroCip30Api) => {
    ...
    return {
        ...
        async signMessage(message: string): Promise<SignResult> {
            ...
            console.log('[gero] signData request', { addr: address, message })
            const rawResponse = await a.signData(addressHex!, payloadHex)
            console.log('[gero] signData raw response', rawResponse)
